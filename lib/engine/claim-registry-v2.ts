...
const CLAIM_AUTHORITY_SET = new WeakSet<object>();
...
function seal(x:Omit<AuthorizedClaim,typeof CLAIM_AUTHORITY>):AuthorizedClaim{const claim=Object.freeze({...x,[CLAIM_AUTHORITY]:true} as AuthorizedClaim);CLAIM_AUTHORITY_SET.add(claim);return claim}
export function isAuthorizedClaim(x:unknown):x is AuthorizedClaim{return !!x&&typeof x==="object"&&CLAIM_AUTHORITY_SET.has(x as object)}
export function isClaimCompatible(claim:AuthorizedClaim,facts:readonly Fact[]):boolean{if(claim.semanticClaimId==="FACT_VALUE")return claim.subjectFactIds.length===1&&facts.some(f=>f.id===claim.subjectFactIds[0]);if(claim.semanticClaimId==="AUTHORIZED_NUMERIC_COMPARISON"){if(claim.claimType!=="COMPARISON_CLAIM"||!claim.comparison||facts.length!==2)return false;const r=ruleFor(facts[0],facts[1]);return !!r&&claim.comparison.temporalBasis===r.temporalBasis&&claim.comparison.valueNature===r.valueNature&&claim.comparison.leftFactId===facts[0].id&&claim.comparison.rightFactId===facts[1].id&&claim.comparison.exactLeftValue===facts[0].value.exactValue&&claim.comparison.exactRightValue===facts[1].value.exactValue&&claim.comparison.unit===facts[0].value.unit&&claim.comparison.unit===facts[1].value.unit&&claim.comparison.completenessCompatibility===true&&claim.comparison.relation===(facts[0].value.exactValue===facts[1].value.exactValue?"EQUAL":facts[0].value.exactValue>facts[1].value.exactValue?"GREATER_THAN":"LESS_THAN")}const allowed=semanticCompatibility[claim.semanticClaimId];return !!allowed&&claim.subjectFactIds.every(id=>{const f=facts.find(x=>x.id===id);return !!f&&allowed.includes(f.semanticId)})}
