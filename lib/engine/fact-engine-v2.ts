import type { VehiclePathReference } from "../reference-data/types";
...
export function buildAccumulationFacts(inputData:{currentResources:number;monthlyContribution:number;projectHorizonMonths:number;withoutYield:number;withDiReference:number|null;diReference?:VehiclePathReference}):Fact[]{
...
export function buildFinancingFacts(d:{vehicleReferenceValue:number;allocatedDownPayment:number;productTermMonths:number;principal:number;payment:number;installmentsTotal:number;projectedOutlay:number;mathematicalInterest:number;rateReference:VehiclePathReference}):Fact[]{
  ...
  const rate=source("fin.rate","FINANCING_AVERAGE_MONTHLY_RATE",d.rateReference,"CURRENT",["AVERAGE_REFERENCE_NOT_USER_RATE","NOT_CET","NOT_AN_OFFER","NOT_A_QUOTE"]);
  ...
  return [vehicle,down,term,rate,calc("fin.principal","FINANCING_PRINCIPAL",d.principal,"PV=vehicle-downPayment",[vehicle.id,down.id]),calc("fin.payment","FINANCING_MATHEMATICAL_PRICE_INSTALLMENT",d.payment,"PRICE_PMT",p),calc("fin.installments","FINANCING_INSTALLMENTS_TOTAL",d.installmentsTotal,"PMT*n",p),calc("fin.outlay","FINANCING_PROJECTED_OUTLAY",d.projectedOutlay,"downPayment+PMT*n",p),calc("fin.interest","FINANCING_MATHEMATICAL_INTEREST",d.mathematicalInterest,"installmentsTotal-principal",p)];
}

export function buildConsortiumFacts(d:{creditReference:number;productTermMonths:number;administrationReference:number;baseSimulatedTotal:number;baseMathematicalInstallment:number;adminFeeReference:VehiclePathReference}):Fact[]{
  ...
  const rate=source("con.admin-rate","CONSORTIUM_STATISTICAL_ADMIN_FEE_RATE",d.adminFeeReference,"DATED",["STATISTICAL_REFERENCE","DATED_REFERENCE","NOT_AN_OFFER","NOT_A_QUOTE","CONTRACT_CONDITIONS_MAY_DIFFER","UNKNOWN_COSTS_NOT_ASSUMED"]);
